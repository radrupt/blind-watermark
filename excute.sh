...
# place 和sceonds需一致，统一数值的不同格式
place=00:00:07
seconds=7
PWD=$(pwd)
# 添加盲水印
# 获取4.4s的图片(后面考虑优化为第多少帧)
$ffmpegPath -i $testVideo -y -f image2 -ss $place -vframes 1 test/test.jpg
# 加载用于生成盲水印的docker
...
# 放回原视频
$ffmpegPath -i $testVideo -i test/test1.jpg -filter_complex "[1]setpts=${seconds}/TB[im];[0][im]overlay=eof_action=pass" -c:a copy -y $outVideo

# 恢复盲水印
# 从视频中获取4.4图片
$ffmpegPath -i $outVideo -y -f image2 -ss $place -vframes 1 test/daoban.jpg
